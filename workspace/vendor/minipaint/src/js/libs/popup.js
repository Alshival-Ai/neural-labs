...
class Dialog_class {
	...
	show_action() {
		...
		// Neural Labs: CSP forbids inline script handlers in the opaque editor.
		this.el.querySelector('[data-id="params_content"]').innerHTML = html_params.replace(/\son(?:change|click|input)="[^"]*"/g, '');
		this.el.querySelectorAll('input,select,textarea').forEach((input) => {
			...
			if (input.type === 'range') input.addEventListener('input', () => {
				const value = input.closest('tr').querySelector('.range_value');
				if (value) value.textContent = Math.round(Number(input.value) * 100) / 100;
