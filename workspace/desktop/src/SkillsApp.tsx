...
export type SkillDraftSummary = { id: string; title: string; kind: "skill" | "automation"; ownerDisplayName: string; updatedAt: string; publishedAt?: string; canManageCollaborators?: boolean; administrator?: boolean };
...
export type SkillsAppProps = {
  ...
  onEditSkill?: (skill: SkillRecord) => void; onDuplicateSkill?: (skill: SkillRecord) => Promise<SkillRecord | void> | void;
  onDeleteSkill?: (skill: SkillRecord) => Promise<unknown>; canDeleteSkill?: (skill: SkillRecord) => boolean;
  onDuplicateDraft?: (draft: SkillDraftSummary) => Promise<unknown>; onDeleteDraft?: (draft: SkillDraftSummary) => Promise<unknown>;
};
...
export function SkillsApp({ skills = PLACEHOLDER_SKILLS, clawHubResults = PLACEHOLDER_CLAWHUB_RESULTS, workspaceName = "Workspace", currentUserName = "You", currentUserId, gatewayOnline = true, canInstallFromOpenClaw = true, loading = false, error, onRefresh, onSelectSkill, onSelectHub, onDiscoverSearch, onInvoke, onSave, onShare, onInstall, drafts = [], automationsContent, initialSection = "mine", sectionRequestId, onCreateSkill, onCreateAutomation, onOpenDraft, onEditSkill, onDuplicateSkill, onDeleteSkill, canDeleteSkill, onDuplicateDraft, onDeleteDraft }: SkillsAppProps) {
  const libraryResize = useLibraryResize(currentUserId, "skills-list-width", 288);
  ...
  const duplicateSkill = async (skill: SkillRecord) => {const copy=await onDuplicateSkill?.(skill);if(copy){setSection("mine");setQuery("");setSelectedSkillId(copy.id);setMobileDetail(true);}setNotice(`${skill.name} copied.`);};
  const skillActions = (skill: SkillRecord): ItemAction[] => [
    {label:"Open",run:()=>selectSkill(skill)},
    {label:"Edit",disabled:!skill.editable||!onEditSkill,reason:!skill.editable?"Duplicate this skill to edit your own copy":undefined,run:()=>onEditSkill?.(skill)},
    {label:"Use in Neura",disabled:!onInvoke||!skill.enabled||skill.eligibility!=="eligible",run:()=>onInvoke?.(skill)},
    {label:"Duplicate",disabled:!onDuplicateSkill,run:()=>duplicateSkill(skill)},
    ...(skill.custom&&skill.ownedByCurrentUser&&onShare?[{label:skill.scope==='personal'?"Share with team":"Make personal",run:()=>changeScope(skill)}]:[]),
    {label:"Delete",danger:true,disabled:!onDeleteSkill||!canDeleteSkill?.(skill),reason:"Only owners and Team administrators can delete writable skills",confirm:skill.scope==='personal'?"This removes the saved skill package. Existing drafts and automations remain.":"This removes the skill for everyone in the workspace. Automations referencing it may need updating. Existing drafts remain.",run:async()=>{await onDeleteSkill?.(skill);setNotice(`${skill.name} deleted.`);}},
  ];
  const draftActions = (draft: SkillDraftSummary): ItemAction[] => [
    {label:"Open",run:()=>onOpenDraft?.(draft)},
    {label:"Duplicate draft",disabled:!onDuplicateDraft||(draft.kind==='automation'&&!draft.administrator),run:()=>onDuplicateDraft?.(draft)},
    {label:"Delete draft",danger:true,disabled:!onDeleteDraft||!draft.canManageCollaborators,reason:"Only the owner or an administrator can delete this draft",confirm:"This deletes the unpublished draft and disconnects its collaborators. Published skills and automations remain.",run:()=>onDeleteDraft?.(draft)},
  ];
  ...
  return <div className="skills-app">
    <header className="skills-toolbar">
      <div className="skills-toolbar__identity"><span><Blocks /></span><div><strong>Skills &amp; Automations</strong><small>{workspaceName}</small></div></div>

      <div className={`skills-toolbar__gateway${gatewayOnline ? " is-online" : ""}`}><i /><span>{gatewayOnline ? "Synced" : "OpenClaw offline"}</span></div>
      {section !== "automations" && <button type="button" className="skills-icon-button" aria-label="Refresh skills" disabled={busy} onClick={() => void Promise.resolve(onRefresh?.()).then(() => setNotice("Skills refreshed.")).catch(() => undefined)}><RefreshCw /></button>}
      {section !== "automations" && (onCreateSkill || onSave) && <button type="button" className="skills-primary-button" onClick={() => onCreateSkill ? onCreateSkill() : (setEditorSkill(undefined), setEditorOpen(true))}><Plus />New skill</button>}
      {section === "automations" && onCreateAutomation && <button type="button" className="skills-primary-button is-automation" onClick={onCreateAutomation}><Plus />New automation</button>}
    </header>
    ...
    <div className="skills-shell">
      <aside className="skills-navigation" aria-label="Skills navigation"><div className="skills-navigation__label">Choose a library</div><nav>{NAVIGATION.map((item) => { const Icon = item.icon; const count = item.id === "mine" ? groups.mine.length : item.id === "team" ? groups.team.length : item.id === "drafts" ? drafts.length : item.id === "automations" ? undefined : groups.openclaw.length; return <TitleTooltip key={item.id} title={item.label}><button aria-label={item.label} type="button" className={`is-${item.accent}`} aria-current={section === item.id ? "page" : undefined} onClick={() => navigate(item.id)}><span><Icon /></span><div><strong>{item.label}</strong><small>{item.description}</small></div>{count !== undefined && <em>{count}</em>}</button></TitleTooltip>; })}</nav><div className="skills-navigation__scope"><LockKeyhole /><div><strong>Personal by default</strong><span>Your new skills appear only in your Neura picker until you share them.</span></div></div><div className="skills-navigation__footer"><CircleCheck /><span>{readyCount} ready</span></div></aside>
      <main className="skills-main">
        {(section === "mine" || section === "team") && <div ref={libraryResize.containerRef} style={libraryResize.style} className={`skills-library${mobileDetail ? " has-mobile-detail" : ""}`}>
          <aside className="skills-library-list" aria-label={`${section === "mine" ? "My" : "Team"} skill library`}><div className="skills-list-heading"><div><span>{section === "mine" ? "Personal" : workspaceName}</span><strong>{section === "mine" ? `${currentUserName}'s skills` : "Team Skills"}</strong></div></div><label className="skills-search"><Search /><span className="skills-sr-only">Search skills</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search skills…" /></label><div className="skills-simple-note">{section === "mine" ? "Only these appear in your personal picker." : "Everyone can find and use these."}</div><SkillList actions={skillActions} skills={visibleSkills} selectedId={selectedSkill?.id} onSelect={selectSkill} empty={section === "mine" ? "Create your first personal skill" : "No Team Skills yet"} /><footer className="skills-library-list__footer"><RefreshCw /><span>Changes apply to new Neura turns</span></footer>{libraryResize.separator}</aside>
          {selectedSkill ? <SkillDetail skill={selectedSkill} busy={busy} onBack={() => setMobileDetail(false)} onInvoke={() => { onInvoke?.(selectedSkill); setNotice(`${selectedSkill.command ?? `$${selectedSkill.key}`} added to Neura.`); }} onEdit={selectedSkill.editable && (onEditSkill || onSave) ? () => onEditSkill ? onEditSkill(selectedSkill) : (setEditorSkill(selectedSkill), setEditorOpen(true)) : undefined} onDuplicate={onDuplicateSkill ? () => { void duplicateSkill(selectedSkill).catch(e=>setNotice(String(e))); } : undefined} onChangeScope={selectedSkill.custom && selectedSkill.ownedByCurrentUser && onShare ? () => void changeScope(selectedSkill) : undefined} /> : <EmptyDetail icon={section === "mine" ? UserRound : Users} title={section === "mine" ? "No personal skills yet" : "No team skills yet"} copy={section === "mine" ? "Create a skill to reuse instructions in Neura." : "Share a personal skill when it becomes useful to everyone."} action={onCreateSkill ?? (onSave ? () => { setEditorSkill(undefined); setEditorOpen(true); } : undefined)} />}
        </div>}
        {section === "drafts" && <div className="skills-drafts"><header><div><span>Collaborative workspace</span><h1>Drafts</h1><p>Autosaved packages and automations. All administrators can inspect every draft.</p></div><button type="button" onClick={onCreateSkill}><Plus /> Skill</button></header><div>{drafts.map((draft) => <ItemActions key={draft.id} name={draft.title} actions={draftActions(draft)}><TitleTooltip title={draft.title}><button type="button" onClick={() => onOpenDraft?.(draft)}><span className={`is-${draft.kind}`}>{draft.kind === "skill" ? <Bot /> : <CalendarClock />}</span><div><strong>{draft.title}</strong><small>{draft.kind} · {draft.ownerDisplayName}</small><em>Updated {new Date(draft.updatedAt).toLocaleString()}</em></div><ChevronRight /></button></TitleTooltip></ItemActions>)}{drafts.length === 0 && <div className="skills-drafts__empty"><FileCode2 /><strong>No drafts yet</strong><span>Create a skill to open the collaborative builder.</span></div>}</div></div>}
...
function SkillList({ skills, selectedId, onSelect, empty, actions }: { actions: (skill:SkillRecord)=>ItemAction[]; skills: readonly SkillRecord[]; selectedId?: string; onSelect: (skill: SkillRecord) => void; empty: string }) {
  return <div className="skills-list-scroll">{skills.map((skill) => <ItemActions key={skill.id} name={skill.name} actions={actions(skill)}><TitleTooltip title={skill.name}><button type="button" className={`skill-list-item is-${skill.accent}${selectedId === skill.id ? " is-selected" : ""}`} aria-label={skill.name} onClick={() => onSelect(skill)}><span className="skill-list-item__mark">{skill.emoji}</span><span className="skill-list-item__copy"><strong>{skill.name}</strong><small>{SOURCE_LABELS[skill.source]} · {skill.command ?? "automatic"}</small><em className={`is-${skill.eligibility}`}><i />{STATUS_COPY[skill.eligibility]}</em></span><ChevronRight /></button></TitleTooltip></ItemActions>)}{skills.length === 0 && <div className="skills-list-empty"><Sparkles /><strong>{empty}</strong><span>Use New skill to get started.</span></div>}</div>;
}
