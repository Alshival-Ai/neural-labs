...
export function SkillsApp({ skills = PLACEHOLDER_SKILLS, clawHubResults = PLACEHOLDER_CLAWHUB_RESULTS, workspaceName = "Workspace", currentUserName = "You", gatewayOnline = true, canInstallFromOpenClaw = true, loading = false, error, onRefresh, onSelectSkill, onSelectHub, onDiscoverSearch, onInvoke, onSave, onShare, onInstall, drafts = [], automationsContent, initialSection = "mine", sectionRequestId, onCreateSkill, onCreateAutomation, onOpenDraft, onEditSkill, onDuplicateSkill, onDeleteSkill, canDeleteSkill, onDuplicateDraft, onDeleteDraft }: SkillsAppProps) {
  ...
  return <div className="skills-app">
    <header className="skills-toolbar">
      ...
      {(onCreateSkill || onSave) && <button type="button" className="skills-primary-button" onClick={() => onCreateSkill ? onCreateSkill() : (setEditorSkill(undefined), setEditorOpen(true))}><Plus />New skill</button>}
...
function SkillList({ skills, selectedId, onSelect, empty, actions }: { actions: (skill:SkillRecord)=>ItemAction[]; skills: readonly SkillRecord[]; selectedId?: string; onSelect: (skill: SkillRecord) => void; empty: string }) {
  return <div className="skills-list-scroll">{skills.map((skill) => <ItemActions key={skill.id} name={skill.name} actions={actions(skill)}><button type="button" className={`skill-list-item is-${skill.accent}${selectedId === skill.id ? " is-selected" : ""}`} aria-label={skill.name} onClick={() => onSelect(skill)}><span className="skill-list-item__mark">{skill.emoji}</span><span className="skill-list-item__copy"><strong>{skill.name}</strong><small>{SOURCE_LABELS[skill.source]} · {skill.command ?? "automatic"}</small><em className={`is-${skill.eligibility}`}><i />{STATUS_COPY[skill.eligibility]}</em></span><ChevronRight /></button></ItemActions>)}{skills.length === 0 && <div className="skills-list-empty"><Sparkles /><strong>{empty}</strong><span>Use New skill to get started.</span></div>}</div>;
}
