import {
  ...
  CloudDownload, Code2, FileCode2, Folder, Globe2, KeyRound, LockKeyhole, PackageSearch,
...
export type SkillsSection = "mine" | "team" | "openclaw";
...
export const PLACEHOLDER_SKILLS: readonly SkillRecord[] = [
  {
    ...
    userInvocable: true, modelInvocable: false, command: "$customer_handoff", writable: true, custom: true,
  ...
  },
  {
    ...
    userInvocable: true, modelInvocable: true, command: "$release_notes", writable: true, custom: true,
  ...
  },
  {
    ...
    userInvocable: true, modelInvocable: true, command: "$image_lab", writable: true, custom: true, editable: true,
...
export function SkillsApp({ skills = PLACEHOLDER_SKILLS, clawHubResults = PLACEHOLDER_CLAWHUB_RESULTS, workspaceName = "Workspace", currentUserName = "You", gatewayOnline = true, canInstallFromOpenClaw = true, loading = false, error, onRefresh, onSelectSkill, onSelectHub, onDiscoverSearch, onInvoke, onSave, onShare, onInstall }: SkillsAppProps) {
  const [section, setSection] = useState<SkillsSection>("mine");
  ...
  const visibleSkills = useMemo(() => {
    ...
    const rows = section === "mine" ? groups.mine : section === "team" ? groups.team : groups.openclaw;
  ...
  return <div className="skills-app">
    <header className="skills-toolbar">
      ...
      {onSave && <button type="button" className="skills-primary-button" onClick={() => { setEditorSkill(undefined); setEditorOpen(true); }}><Plus /> New skill</button>}
    </header>
    ...
    <div className="skills-shell">
      <aside className="skills-navigation" aria-label="Skills navigation"><div className="skills-navigation__label">Choose a library</div><nav>{NAVIGATION.map((item) => { const Icon = item.icon; const count = item.id === "mine" ? groups.mine.length : item.id === "team" ? groups.team.length : groups.openclaw.length; return <button key={item.id} type="button" className={`is-${item.accent}`} aria-current={section === item.id ? "page" : undefined} onClick={() => navigate(item.id)}><span><Icon /></span><div><strong>{item.label}</strong><small>{item.description}</small></div><em>{count}</em></button>; })}</nav><div className="skills-navigation__scope"><LockKeyhole /><div><strong>Personal by default</strong><span>Your new skills appear only in your Neura picker until you share them.</span></div></div><div className="skills-navigation__footer"><CircleCheck /><span>{readyCount} ready</span></div></aside>
      <main className="skills-main">
        {section !== "openclaw" && <div className={`skills-library${mobileDetail ? " has-mobile-detail" : ""}`}>
          ...
          {selectedSkill ? <SkillDetail skill={selectedSkill} busy={busy} onBack={() => setMobileDetail(false)} onInvoke={() => { onInvoke?.(selectedSkill); setNotice(`${selectedSkill.command ?? `$${selectedSkill.key.replaceAll("-", "_")}`} added to Neura.`); }} onEdit={selectedSkill.editable && onSave ? () => { setEditorSkill(selectedSkill); setEditorOpen(true); } : undefined} onChangeScope={selectedSkill.custom && selectedSkill.editable && onShare ? () => void changeScope(selectedSkill) : undefined} /> : <EmptyDetail icon={section === "mine" ? UserRound : Users} title={section === "mine" ? "Your own way of working" : "Shared ways of working"} copy={section === "mine" ? "Create a skill and it is ready in Neura—no approval queue." : "Share a personal skill when it becomes useful to everyone."} action={onSave ? () => { setEditorSkill(undefined); setEditorOpen(true); } : undefined} />}
        </div>}
