...
type Policy = { provider: "openai" | "anthropic"; mode: "latest" | "pinned"; model: string; effort: string };
      ...
      {policy && <>
        {compact ? <>
          <div className="provider-defaults-grid">
            <label><span>Default provider</span><select aria-label="Default provider" value={policy.provider} disabled={busy} onChange={event => setPolicy({ provider: event.target.value as Policy["provider"], mode: "latest", model: "", effort: "" })}><option value="openai">OpenAI</option><option value="anthropic">Claude</option></select></label>
            <ModelPicker catalog={catalog && { ...catalog, models: catalog.models.filter(row => row.provider === policy.provider), defaultModel: selectedModel || catalog.defaultModel }} model={policy.mode === "latest" ? "" : policy.model} effort={policy.effort} disabled={busy} defaultLabel="Latest recommended model" effortDefaultLabel="Model default" onChange={(model, effort) => setPolicy({ ...policy, provider: model.startsWith("anthropic/") ? "anthropic" : model ? "openai" : policy.provider, mode: model ? "pinned" : "latest", model, effort })} />
          </div>
        ...
        </> : <><label><span>Provider</span><select aria-label="Provider" value={policy.provider} disabled={busy} onChange={event => setPolicy({ provider: event.target.value as Policy["provider"], mode: "latest", model: "", effort: "" })}><option value="openai">OpenAI</option><option value="anthropic">Claude</option></select></label><label><span>Model selection</span><select aria-label="Model selection" disabled={busy} value={policy.mode} onChange={(event) => setPolicy({ ...policy, mode: event.target.value as Policy["mode"], model: selectedModel, effort: "" })}><option value="latest">Follow latest compatible recommendation</option><option value="pinned">Pin a specific model</option></select></label>
        {policy.mode === "pinned" ? <ModelPicker catalog={catalog} model={policy.model} effort={policy.effort} disabled={busy} defaultLabel="Choose a model" effortDefaultLabel="Model default" onChange={(model, effort) => setPolicy({ ...policy, provider: model.startsWith("anthropic/") ? "anthropic" : "openai", model, effort })} /> : <>
