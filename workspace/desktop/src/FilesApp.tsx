import {
  Archive, ArrowUpDown, ChevronRight, Clock3, Cloud, Code2, Download, File,
...
export function FilesApp({ notify, onOpenFile, onPreviewFile, storageNamespace, storageArea = "files" }: FilesAppProps) {
  ...
  const openContextMenu = (event: MouseEvent, item: WorkspaceFile) => {
    ...
    setContextMenu({ item, x: Math.min(event.clientX, window.innerWidth - 190), y: Math.min(event.clientY, window.innerHeight - 145) });
  };
