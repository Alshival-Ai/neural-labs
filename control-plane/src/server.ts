...
const claudeConnectionSchema = z.object({
  ...
  state: z.enum(["disconnected", "connected", "awaiting_user", "error"]), message: z.string().max(500).nullable(), attemptId: z.string().uuid().optional(), terminalId: z.string().uuid().optional(),
});
...
export function createApplication(input: {
  ...
  for (const scope of ["account", "admin/workspace"] as const) {
    ...
    const handler = async (request: Request, response: Response) => {
      ...
      if (action && !["connect", "cancel", "pause", "resume", "disconnect", "refresh", "api-key"].includes(action)) { jsonError(response, 404, "not_found", "Unknown connection action"); return; }
      if (action === "api-key" && scope === "account") { jsonError(response, 403, "forbidden", "Workspace administrator access is required"); return; }
      if (action && !await database.consumeRateLimit(`claude-action:${actor.user.id}`, 20, 60)) { jsonError(response, 429, "rate_limited", "Wait a moment and try again."); return; }
      ...
      try {
        ...
        response.json(claudeConnectionSchema.parse(await result.json()));
